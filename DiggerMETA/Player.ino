...
void updateCursor() {
  if (gb.buttons.repeat(BUTTON_A, 4) && !dead) {
    gb.sound.playOK();
    die();
  }

  if (gb.buttons.repeat(BUTTON_B, 4) && dead ) {
    gb.sound.playCancel();
    initWorld(curLevel);
    dead = false;
  }
  ...
  int camera_x_target = cursor_x * 6 - gb.display.width() / 2 + 3;
  int camera_y_target = cursor_y * 6 - gb.display.height() / 2 + 3;
